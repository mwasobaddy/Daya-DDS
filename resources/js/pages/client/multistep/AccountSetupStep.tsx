...
export default function AccountSetupStep({ value, onChange, onNext }: Props) {
  const [companyName, setCompanyName] = useState(String(value.companyName ?? ''));
  const [contactPerson, setContactPerson] = useState(String(value.contactPerson ?? ''));
    ...
    <form className="space-y-6" onSubmit={e => { e.preventDefault(); handleNext(); }}>
      <div className="space-y-5">
        ...
        <div className="grid gap-2">
          ...
          />
          <InputError message={errors.contactPerson} />
        </div>
